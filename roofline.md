...
当模型大到一张卡放不下，需要**多卡协作**时，数据还得在卡之间传来传去。常见的连接方式有 ICI、DCN、NVLink、PCIe 等，各有不同的带宽。
...
假设 HBM 带宽 `8.2×10¹¹` 字节/s，int8 峰值 `3.94×10¹⁴` OPs/s。

{% details 点击查看答案 %}

1. 加载 $BD + DF$ 字节，写回 $BF$ 字节
2. 还是 $2BDF$ FLOPs（只是精度变了）
3. 强度 ≈ $2B$，临界值 = `3.94×10¹⁴ / 8.2×10¹¹` ≈ 480，所以 $B > 240$ 时计算受限。跟 bf16 差不多！
4. $T_\text{计算} = 2BDF / 3.94×10^{14}$，$T_\text{通信} = (BD + DF + BF) / 8.2×10^{11}$
