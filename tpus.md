...
**一个 TPU 芯片通常有两个核心，共享内存**（叫"megacore"模式，v4、v5、v6 都是这样；TPU v7 取消了 megacore，改为两个核心之间的高带宽互连）。老款 TPU（v3 及以前）的两个核心是独立的。推理芯片（如 v5e）每个芯片只有一个核心。
...
| 连接类型 | 带宽（每芯片） | 备注 |
|---------|---------------|------|
| HBM ↔ VMEM | 2.8 TB/s | 最快 |
| ICI（每轴） | 90 GB/s（双向）<d-footnote>单向 45 GB/s × 2。完整环才能用满双向。</d-footnote> | v5p 有 3 轴 |
| PCIe | ~16 GB/s | 比 HBM 慢 100 倍 |
| DCN（出口） | ~6 GB/s<d-footnote>v6e 和 TPU7x 是 12.5 GB/s，v5e 是 3.125 GB/s。</d-footnote> | 最慢 |
...
| v7x  | 4×4×576  | 2×2×1  | 192GB    | 7.4 TB/s | 2300 TF/s | 4610 TF/s |

*TF/s = 10¹² FLOPs/s*

关于最新一代 TPU 的详细信息，参见 [TPU7x 文档](https://docs.cloud.google.com/tpu/docs/tpu7x)。
...
| v7x  | 90 GB/s  | 180 GB/s |

<d-footnote>双向带宽 = 单向 × 2，完整环时可以用满。</d-footnote>

PCIe 约 16 GB/s/芯片（v6e 是 32），DCN 约 6 GB/s/芯片（v6e 和 v7x 是 12.5，v5e 是 3.125）。
...
- 算力：8960 × 4.59×10¹⁴ = **4.1×10¹⁸ bf16 FLOPs/s**（4.1 EF）
- 显存：8960 × 96GB = **860TB**
...
假设权重 `bf16[D, 4D]` 和激活 `bf16[B, D]` 都存在主机内存（不在 TPU 显存），你想用一张 TPU v6e 算矩阵乘法。假设 $B \ll D$。需要多大 batch 才能计算受限？（PCIe 带宽 1.6×10¹⁰ 字节/s）
...
计算受限条件：
$$\frac{8BD^2}{9.2×10^{14}} > \frac{8D^2}{1.6×10^{10}}$$

$$B > \frac{9.2×10^{14}}{1.6×10^{10}} ≈ 57500$$

需要近 **6 万的 batch** 才能计算受限！PCIe 太慢了。
...
- $T_\text{通信} = (6.7×10^7 + 2×10^4×B) / 8.2×10^{11}$

计算受限条件：$B > 267$（比[第 1 章](../roofline)算出的 240 稍大，因为这里考虑了 D 和 F 的完整影响）
...
总共约 **188 + 6 = 194μs**（带宽受限，加上最后一个字节同样需要经过 6 跳的延迟；一般来说，延迟和带宽项是可加的，但这里延迟只是一个小修正）。
...
1. **PCIe 加载**：16GB / 16 芯片 = 1GB/芯片，带宽 1.6×10¹⁰ → 约 **63ms**

2. **ICI 收集**：TPU(0,0) 要收 15GB，2 个方向各 45 GB/s → 下界 **167ms**（实际可能更长）

3. **HBM → MXU**：16GB / 8.2×10¹¹ ≈ **20ms**
...
{% details 答案 %}
每核每周期：$8 × 128 × 4$ = 4096 FLOPs
每核总算力：$4096 × 1.75×10^9 = 7×10^{12}$ FLOPs/s

比每核 MXU 算力（约 2×10¹⁴）小约 30 倍。
